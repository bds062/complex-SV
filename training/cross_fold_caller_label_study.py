"""Grouped cross-fold caller-label evaluation for candidate-region models.

This trains the general candidate-region classifier on grouped cell-line folds
and evaluates each held-out candidate exactly once against the external caller
labels used to build the training set.
"""
...
def run(args: argparse.Namespace) -> None:
    ...
    config = {
        ...
        "fast_thresholds": bool(args.fast_thresholds),
        "epochs_override": int(args.epochs) if args.epochs is not None else None,
        "patience_override": int(args.patience) if args.patience is not None else None,
    ...
    for fold_index, fold_id in enumerate(unique_folds, start=1):
        ...
        try:
            ...
            torch.save(
                ...
                run_dir / "candidate_region_classifier.pt",
            )
    ...
    overview = {
        "evaluation_mode": "grouped_cross_fold",
    ...
    plot_fold_heatmap(fold_per_class, figures_dir, class_names)
    plot_poster_summary(aggregate_class, fold_summary, figures_dir)
    log.info("Done. Wrote cross-fold caller-label study to %s", output_dir)
