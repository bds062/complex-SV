"""Grouped cross-fold hybrid evaluation: neural objectness plus few-shot types.

For each shared held-out fold, raw neural objectness probabilities are combined
with raw few-shot type probabilities. All hybrid thresholds are calibrated from
the corresponding fold training candidates only.
"""
...
def run(args: argparse.Namespace) -> None:
    ...
    config = {
        ...
        "model_type": "candidate_region_hybrid_neural_objectness_fewshot_type",
        "objectness_source_dir": str(source_dir),
        "fewshot_type_source_dir": str(args.fewshot_source_dir),
        "type_source": "few-shot prototype probabilities",
    ...
    for fold_index, fold_id in enumerate(unique_folds, start=1):
        ...
        try:
            ...
            neural_raw_path = source_dir / "runs" / fold_name / "raw_predictions.tsv"
            fewshot_raw_path = Path(args.fewshot_source_dir) / "runs" / fold_name / "cluster_aggregated_raw_predictions.tsv"
            if not neural_raw_path.exists():
                raise FileNotFoundError(f"Missing neural raw predictions: {neural_raw_path}")
            if not fewshot_raw_path.exists():
                raise FileNotFoundError(f"Missing few-shot raw predictions: {fewshot_raw_path}")
            neural_raw = pd.read_csv(neural_raw_path, sep="\t").fillna("")
            fewshot_raw = pd.read_csv(fewshot_raw_path, sep="\t").fillna("")
            for frame, source_name in [(neural_raw, "neural"), (fewshot_raw, "few-shot")]:
                if "candidate_id" not in frame:
                    raise ValueError(f"{source_name} fold output lacks candidate_id: {fold_name}")
                if frame["candidate_id"].astype(str).duplicated().any():
                    raise ValueError(f"{source_name} fold output has duplicate candidate IDs: {fold_name}")
            expected_ids = metadata["candidate_id"].astype(str).tolist()
            neural_by_id = neural_raw.set_index(neural_raw["candidate_id"].astype(str))
            fewshot_by_id = fewshot_raw.set_index(fewshot_raw["candidate_id"].astype(str))
            if set(neural_by_id.index) != set(expected_ids) or set(fewshot_by_id.index) != set(expected_ids):
                raise ValueError(f"Raw prediction IDs do not match the shared embedding corpus in {fold_name}")
            raw = fewshot_by_id.loc[expected_ids].copy()
            raw["neural_objectness_prob"] = pd.to_numeric(neural_by_id.loc[expected_ids, "objectness_prob"], errors="coerce").to_numpy()
            raw["neural_objectness_logit"] = pd.to_numeric(neural_by_id.loc[expected_ids, "objectness_logit"], errors="coerce").to_numpy()
            raw["objectness_prob"] = raw["neural_objectness_prob"].astype(float)
            raw["objectness_logit"] = raw["neural_objectness_logit"].astype(float)
            raw.index.name = "candidate_id"
            ...
            torch.save(
                {
                    ...
                    "neural_raw_predictions": str(neural_raw_path),
                    "fewshot_raw_predictions": str(fewshot_raw_path),
                    **threshold_info,
                },
                run_dir / "candidate_region_classifier_hybrid.pt",
            )
    ...
    overview = {
        "evaluation_mode": "grouped_cross_fold_hybrid",
        ...
        "hybrid_sources": "neural objectness probabilities plus few-shot type probabilities",
    }
    ...
    plot_fold_heatmap(fold_per_class, figures_dir, class_names)
    plot_poster_summary(aggregate_class, fold_summary, figures_dir)
    log.info("Done. Wrote grouped cross-fold hybrid caller-label study to %s", output_dir)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", default="/data/KolmogorovLab/srinivasanbd/results/pipeline14/candidate_region_classifier_hybrid_general_fewshot")
    ...
    parser.add_argument("--fewshot_source_dir", default="/data/KolmogorovLab/srinivasanbd/results/pipeline14/candidate_region_classifier_fewshot_general")
