...
if [[ $# -lt 2 ]]; then
    echo "Usage: $0 MANIFEST.tsv OUTPUT_DIR [additional embedding.py arguments]" >&2
    exit 2
fi
...
"$python_bin" "$repo_dir/genomic_features/embedding.py" \
    --manifest "$manifest" \
...
"$python_bin" "$repo_dir/scripts/build_chromosome_tabular.py" \
    --manifest "$manifest" \
