...
# Apply pipeline5's few-shot prototypical classifier to another manifest.
# Default example: pipeline5-trained few-shot head applied to pipeline6.
...
OUTPUT_DIR="${OUTPUT_DIR:-$PROTOTYPE_ROOT/fewshot_classification_head_applied}"
PLOT_DIR="${PLOT_DIR:-$OUTPUT_DIR/predicted_chromosome_arm_plots}"
CHECKPOINT="${CHECKPOINT:-../results/pipeline5/prototype_chrom_arm_sample_norm/fewshot_classification_head/fewshot_classification_head.pt}"
...
if ! "$PYTHON_BIN" -c "import pandas, torch, matplotlib, torch_geometric" >/dev/null 2>&1; then
    echo "[apply_fewshot_classification_head] ERROR: $PYTHON_BIN cannot import pandas, torch, matplotlib, and torch_geometric." >&2
    exit 1
fi

for required_file in "$MANIFEST" "$CHECKPOINT"; do
    if [[ ! -f "$required_file" ]]; then
        echo "[apply_fewshot_classification_head] ERROR: required file not found: $required_file" >&2
        exit 1
    fi
done
...
if [[ "$FORCE_EMBED" == "1" || ! -f "$EMBEDDINGS_NPZ" || ! -f "$METADATA_TSV" ]]; then
    if [[ ! -f "$CN_CHECKPOINT" ]]; then
        echo "[apply_fewshot_classification_head] ERROR: required file not found for embedding: $CN_CHECKPOINT" >&2
        exit 1
    fi
    ...
    echo "[apply_fewshot_classification_head] Building embeddings:"
    printf '  %q' "${EMBED_CMD[@]}"
    printf '\n'
    "${EMBED_CMD[@]}"
else
    echo "[apply_fewshot_classification_head] Reusing existing embeddings: $EMBEDDINGS_NPZ"
fi

mkdir -p "$OUTPUT_DIR"
APPLY_CMD=(
    "$PYTHON_BIN" training/apply_fewshot_classifier_head.py
...
if [[ -n "${DISTANCE_TAU:-}" ]]; then
    APPLY_CMD+=(--distance_tau "$DISTANCE_TAU")
fi
if [[ -n "${OBJECTNESS_TAU:-}" ]]; then
    APPLY_CMD+=(--objectness_tau "$OBJECTNESS_TAU")
fi
if [[ -n "${OBJECTNESS_SCALE:-}" ]]; then
    APPLY_CMD+=(--objectness_scale "$OBJECTNESS_SCALE")
fi
if [[ -n "${TEMPERATURE:-}" ]]; then
    APPLY_CMD+=(--temperature "$TEMPERATURE")
fi
if [[ "$SKIP_VISUALIZATIONS" == "1" ]]; then
    APPLY_CMD+=(--skip_visualizations)
fi

echo "[apply_fewshot_classification_head] Applying few-shot classifier:"
printf '  %q' "${APPLY_CMD[@]}"
printf '\n'
"${APPLY_CMD[@]}"
...
if [[ "$PLOT_PREDICTED_CHROMS" == "1" ]]; then
    ...
    echo "[apply_fewshot_classification_head] Plotting predicted chromosome arms:"
    printf '  %q' "${PLOT_CMD[@]}"
    printf '\n'
    "${PLOT_CMD[@]}"
fi
...
for expected_output in \
    ...
    if [[ ! -f "$expected_output" ]]; then
        echo "[apply_fewshot_classification_head] ERROR: expected output not found: $expected_output" >&2
        exit 1
    fi
done
...
if [[ "$PLOT_PREDICTED_CHROMS" == "1" && ! -f "$PLOT_DIR/selected_predictions.tsv" ]]; then
    echo "[apply_fewshot_classification_head] ERROR: expected output not found: $PLOT_DIR/selected_predictions.tsv" >&2
    exit 1
fi

echo "[apply_fewshot_classification_head] Done."
