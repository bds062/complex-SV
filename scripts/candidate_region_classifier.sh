...
SECONDARY_OBJECTIVE="${SECONDARY_OBJECTIVE:-recall}"
...
echo "[candidate_region_classifier] secondary_thresholding=$SECONDARY_THRESHOLDING objective=$SECONDARY_OBJECTIVE min=$SECONDARY_MIN delta=$SECONDARY_DELTA recall_constraint=$SECONDARY_MIN_RECALL precision_constraint=$SECONDARY_MIN_PRECISION"
