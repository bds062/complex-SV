...
# Apply a trained single-label classifier head to a new manifest's chromosome-arm embeddings.
# Default example: pipeline5-trained classification head applied to pipeline6.
...
OUTPUT_DIR="${OUTPUT_DIR:-$PROTOTYPE_ROOT/classification_head_applied}"
PLOT_DIR="${PLOT_DIR:-$OUTPUT_DIR/predicted_chromosome_arm_plots}"
CHECKPOINT="${CHECKPOINT:-../results/pipeline7/prototype_chrom_arm_sample_norm/classification_head/classification_head.pt}"
...
if ! "$PYTHON_BIN" -c "import pandas, torch, matplotlib, torch_geometric" >/dev/null 2>&1; then
    echo "[apply_classification_head] ERROR: $PYTHON_BIN cannot import pandas, torch, matplotlib, and torch_geometric." >&2
    exit 1
fi

for required_file in "$MANIFEST" "$CHECKPOINT"; do
    if [[ ! -f "$required_file" ]]; then
        echo "[apply_classification_head] ERROR: required file not found: $required_file" >&2
        exit 1
    fi
done
...
if [[ "$FORCE_EMBED" == "1" || ! -f "$EMBEDDINGS_NPZ" || ! -f "$METADATA_TSV" ]]; then
    if [[ ! -f "$CN_CHECKPOINT" ]]; then
        echo "[apply_classification_head] ERROR: required file not found for embedding: $CN_CHECKPOINT" >&2
        exit 1
    fi
    ...
    echo "[apply_classification_head] Building embeddings:"
    printf '  %q' "${EMBED_CMD[@]}"
    printf '\n'
    "${EMBED_CMD[@]}"
else
    echo "[apply_classification_head] Reusing existing embeddings: $EMBEDDINGS_NPZ"
fi
...
APPLY_CMD=(
    "$PYTHON_BIN" training/apply_classifier_head.py
...
echo "[apply_classification_head] Applying classifier head:"
printf '  %q' "${APPLY_CMD[@]}"
printf '\n'
"${APPLY_CMD[@]}"
...
if [[ "$PLOT_PREDICTED_CHROMS" == "1" ]]; then
    ...
    echo "[apply_classification_head] Plotting predicted chromosome arms:"
    printf '  %q' "${PLOT_CMD[@]}"
    printf '\n'
    "${PLOT_CMD[@]}"
fi
...
for expected_output in \
    ...
    if [[ ! -f "$expected_output" ]]; then
        echo "[apply_classification_head] ERROR: expected output not found: $expected_output" >&2
        exit 1
    fi
done
...
if [[ "$PLOT_PREDICTED_CHROMS" == "1" && ! -f "$PLOT_DIR/selected_predictions.tsv" ]]; then
    echo "[apply_classification_head] ERROR: expected output not found: $PLOT_DIR/selected_predictions.tsv" >&2
    exit 1
fi

echo "[apply_classification_head] Done."
