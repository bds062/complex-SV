...
This module contains model definitions only.  It is imported by
pretrain_cn.py, fusion-time SVModel components, and post-pretraining
visualisation scripts.

Input convention
----------------
Copy-number tensors use shape [B, n_bins, 5] with channel order:
...
class CNTransformerEncoder(nn.Module):
    ...
    Parameters are read from a config-like object exposing d_model, n_heads,
    n_layers, ff_dim or d_ff, dropout, and n_bins_arm / seq_len.

    forward(x) expects x of shape [B, n_bins, 5] and returns:
    ...
    def __init__(self, cfg: CNEncoderConfig | Any):
        ...
        self.n_bins = int(
            getattr(cfg, "n_bins_arm", getattr(cfg, "seq_len", 256))
        )
        ...
        self.pos_embed = nn.Parameter(
            torch.randn(1, self.n_bins + 1, self.d_model) * 0.02
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if x.ndim != 3 or x.shape[-1] != CN_INPUT_DIM:
            raise ValueError(
                f"CNTransformerEncoder expects [B, L, {CN_INPUT_DIM}], got {tuple(x.shape)}"
            )

        batch_size, seq_len, _ = x.shape
        if seq_len + 1 > self.pos_embed.shape[1]:
            raise ValueError(
                f"Input length {seq_len} exceeds configured maximum {self.pos_embed.shape[1] - 1}"
            )
...
class CNMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, cfg: CNEncoderConfig | Any):
        ...
        # Global reconstruction head from CLS. Useful for novelty scoring.
    ...
    def forward(
        ...
        if x.shape[:2] != mask.shape:
            raise ValueError(
                f"x and mask sequence dimensions must match: x={tuple(x.shape)}, mask={tuple(mask.shape)}"
            )
        ...
        x_masked = torch.where(
            mask.unsqueeze(-1),
            self.mask_token.to(dtype=x.dtype, device=x.device).expand_as(x_masked),
            x_masked,
        )
